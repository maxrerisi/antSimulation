...
SCALE_FACTOR = 1 # the factor by which the display is scaled up
...
PHERMONE_STRENGTH = 100*(SCALE_FACTOR**2) # the number of frames the phermones will exist for
FOOD_TO_REPRODUCE = True # if True, the ants will duplicate when food is brought back to the hill.
AMOUNT_OF_FOOD_TO_REPRODUCE = 3 # the number of food the ant needs to bring back to the hill to reproduce
...
NODE_RADIUS = 50*SCALE_FACTOR # changes the distribution of food within a node
NODE_COUNT, FOOD_PER_NODE = 5, 500
FPS_UPDATE = 3 # the number of frames for the FPS to update
ANT_COUNT = 1 # the number of ants to start with
